...
| 🔍 | **Torrent detail** | Drawer with piece map, per-file progress, health, trackers, comment/creator |
...
| 💾 | **Reboot-proof** | Settings & statistics persist to disk; ships with a ready-made `systemd` unit |
...
├── torrente/   → core engine: piece picker, sessions, rate limits, detail view,
│                 settings (persisted), statistics (detail.go/settings.go/stats.go/trackers.go)
├── web/        → embedded HTTP server (embed.FS) + multilingual UI
│   ├── web.go  → REST API, torrent detail endpoint
│   ├── ws.go   → WebSocket broadcast hub (live 1 Hz ticks)
...
statistics and country logs live in `stats.json`.
...
SHA-1 verification, and seeding confirmed with live peers. Detail endpoint returns
the piece map + per-file progress; the WebSocket handshake and 1 Hz live ticks
verified with a raw `Sec-WebSocket-Key` upgrade.
